<template>
  <div style="padding: 30px">
    ...
    <br />-->
    <h1>Liste des prets en cours</h1>
    <el-row>
    ...
    <!-- deadlines Dialogs -->
    <el-dialog title="Echeances" :visible.sync="deadlinesDialog">
      <div v-if="toChange !== null">
	<!-- <h3>Echéances</h3> -->
	<el-table :data="toChange.repayments" stripe>
	  <el-table-column label="date">
	    <template slot-scope="scope">
	      <span>{{ scope.row.repayment_date | moment("DD MMM YYYY") }}</span>
	    </template>
	  </el-table-column>

	  <el-table-column label="Capital">
	    <template slot-scope="scope">
	      <span>{{ parseFloat(scope.row.capital) }}</span>
	    </template>
	  </el-table-column>

	  <el-table-column label="Interets">
	    <template slot-scope="scope">
	      <span>{{ parseFloat(scope.row.interest) }}</span>
	    </template>
	  </el-table-column>

	  <el-table-column label="Total">
	    <template slot-scope="scope">
	      <span>{{ parseFloat(scope.row.capital) + parseFloat(scope.row.interest) }}</span>
	    </template>
	  </el-table-column>

	  <el-table-column label="Payé?">
	    <template slot-scope="scope">
	      <span>{{ scope.row.paid != true ? 'non' : 'oui' }}</span>
	    </template>
	  </el-table-column>
	</el-table>
      </div>
  ...
  <el-table :data="list" border fit highlight-current-row 
    ...
    <el-table-column min-width="300px" label="Opérations">
      <template slot-scope="scope">
	<el-button size="mini" type="primary" @click="deadlinesDialog = true;toChange = scope.row">Voir les écheances</el-button>
...
export default {
  ...
  methods: {
    getAll() {
      query('loan/listactual', 'GET').then((res) => {
	...
	"reason": this.cancelForm.reason
      ...
      query('loan/delete/' + this.toChange.id, 'POST', data).then((res) => {
	if (res.data.success) {
